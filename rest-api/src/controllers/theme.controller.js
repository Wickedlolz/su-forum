...
router.post("/", isAuth(), async (req, res, next) => {
  ...
  try {
    const theme = await themeService.createTheme(themeName, userId, postText);
...
router.put("/:themeId/posts/:postId", isAuth(), async (req, res, next) => {
  ...
  try {
    const updatedPost = await postService.editPost(postText, userId, postId);
...
router.delete("/:themeId/posts/:postId", isAuth(), async (req, res, next) => {
  ...
  try {
    const deletedPost = await postService.deletePost(postId, themeId, userId);
